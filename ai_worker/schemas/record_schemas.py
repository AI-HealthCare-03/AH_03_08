...
class Medication(BaseModel):
    ...
    concentration: str | None = Field(
        default=None, description="약품 함량/농도 (예: '60mg', '0.2mg'). 약품명에 포함된 용량 정보"
    )
    dosage: float | None = Field(
        default=None, description="1회 복용 정수(개수). 숫자만 (예: 1, 2, 0.5). '1정' → 1, '2캡슐' → 2. mg 농도가 아님"
    )
    frequency: int | None = Field(
        default=None, description="1일 복용 횟수. 숫자만 (예: 3). '1일 3회' → 3, '하루 2번' → 2"
    )
