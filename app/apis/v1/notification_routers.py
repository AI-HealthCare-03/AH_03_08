...
async def list_notifications(current_user=Depends(get_request_user)):
    ...
    return _ok([NotificationResponse(id=str(r.id), medication_id=str(r.medication_id), title=r.title, type=r.type, scheduled_time=str(r.scheduled_time), is_active=r.is_active, created_at=str(r.created_at)) for r in rows])
